...
```{r construct_mtpl1_train_holdout}
...
```{r construct_prior_predictive_data}
...
```{r freq1_calculate_prior_predict_summaries}
...
```{r freq1_calculate_prior_predict_policy_summaries}
...
priorpred_freq2_tbl <- priorpred_input_tbl %>%
  add_linpred_draws(
    object = mtpl1_freq1_prior2_stanreg,
    offset = rep(1, n_sample),
    value  = "freq_mean"
    ) %>%
  ungroup()
...
```{r plot_priorpred_freqmean_max_histogram}
...
```{r freq2_calculate_prior_predict_summaries}
...
priorpred_freq2_policy_tbl %>% datatable(rownames = FALSE)
```
...
ggplot(plot_tbl) +
  geom_boxplot(
    aes(x = policy_id, y = freq_mean, colour = prior_model),
...
priorpred_freq3_tbl <- priorpred_input_tbl %>%
  add_linpred_draws(
    object = mtpl1_freq1_prior3_stanreg,
    offset = rep(1, n_sample),
    value  = "freq_mean"
    ) %>%
  ungroup()
...
```{r compare_nonscaled_autoscaled_prior_params}
...
```{r construct_prior_model_functions}
...
```{r construct_prior_first_model_system}
...
```{r plot_priorpred1_pois_output_sample}
...
```{r plot_priorpred1_negbin_output_sample}
